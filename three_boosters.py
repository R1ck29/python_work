...
train_store_2 = pd.read_csv(input_file_path + '/Train_external_data_2.csv', low_memory=False, skiprows=6, dtype={"Client Id":'str'})
test_store_1 = pd.read_csv(input_file_path + '/Test_external_data.csv', low_memory=False, skiprows=6, dtype={"Client Id":'str'})
...
# Preprocessing
# Setting time as index and saving time as feature
...
def create_user_df(df):
    ...
    pred_list = df[['fullVisitorId', 'lgbpred']].groupby('fullVisitorId').apply(
        lambda visitor_df: list(visitor_df.lgbpred)) \
