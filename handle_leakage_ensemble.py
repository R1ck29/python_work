...
def load_df(csv_path=input_file_path + '/train.csv', nrows=None):
...
train = load_df()
test = load_df(input_file_path + "/test.csv")

# Loading external data
train_store_1 = pd.read_csv(input_file_path + '/Train_external_data.csv', low_memory=False,
                            skiprows=6, dtype={"Client Id": 'str'})
train_store_2 = pd.read_csv(input_file_path + '/Train_external_data_2.csv', low_memory=False,
                            skiprows=6, dtype={"Client Id": 'str'})
test_store_1 = pd.read_csv(input_file_path + '/Test_external_data.csv', low_memory=False,
                           skiprows=6, dtype={"Client Id": 'str'})
test_store_2 = pd.read_csv(input_file_path + '/Test_external_data_2.csv', low_memory=False,
                           skiprows=6, dtype={"Client Id": 'str'})
...
def get_folds(df=None, n_splits=10):  # changed form 5
...
folds = get_folds(df=train, n_splits=10)  # changed from 5
...
print("train.shape[0]の0埋めは" + oof_reg_preds)
print("test.shape[0]の0埋めは" + sub_reg_preds)
...
for fold_, (trn_, val_) in enumerate(folds):
    ...
    reg = lgb.LGBMRegressor(
        num_leaves=31,      # changed from 31
        learning_rate=0.02,  # changed from 0.02
        n_estimators=1000,
        subsample=.9,
        colsample_bytree=.9,
        random_state=1
...
# accuracy=accuracy_score(oof_reg_preds,y_reg)
...
warnings.simplefilter(action='ignore', category=FutureWarning)
folds = get_folds(df=full_data[['totals.pageviews']].reset_index(), n_splits=10)  # changed from 5
...
for fold_, (trn_, val_) in enumerate(folds):
    ...
    reg = lgb.LGBMRegressor(
        num_leaves=31,  # even if it is high, it does not improve score
        learning_rate=0.02,  # 0.02
        n_estimators=1000,
        subsample=.9,
        colsample_bytree=.9,
    ...
    both_oof[val_] = oof_preds[val_] * 0.4 + oof_preds1[val_] * 0.6  # after switched 4:6, 34 places up
    ...
    pre[pre < 0] = 0
    # LightGBM 0.6 + XGBoost 0.4
    sub_preds += (_preds / len(folds)) * 0.4 + (pre / len(folds)) * 0.6  # after switched 4:6, 34 places up
# MSEの0.5乗
...
sub_full_data[['PredictedLogRevenue']].to_csv(output_path + 'ensemble_test_submission_' + today + '.csv', index=True)